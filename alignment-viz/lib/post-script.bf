...
for (_i = 0; _i < Abs(_asciiCharString); _i += 1) {
...
function _HYPSGetStringWidth (s) {
	_w = 0;
	for (_i = 0; _i < Abs(s); _i += 1) {
		_w += _charWidthForTimes[s[_i]];
	}
	return _w;
}
...
function _HYPSTextTable (w,h,fs,tableText,cellBounds) {
	...
	for 	(_i = 0; _i < _rows; _i += 1) {
		for 	(_j = 0; _j < _columns; _j += 1) {
			_strVal = tableText[_i][_j];
			_strLen = Abs(_strVal);
			_cw		= _timesCharMaxWidth;
			for (_k = 0; _k < _strLen; _k += 1) {
...
_hyDefaultGS = {6,3}["(220-_MATRIX_ELEMENT_ROW_*30)/255"];
