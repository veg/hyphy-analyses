...
This version allows test and background branches to have independent synonymous rate distributions, if selected.
",
...
busted.SRV.background = "Synonymous site-to-site rates (background)";
...
busted.do_srv = io.SelectAnOption ({"Yes" : "Allow synonymous substitution rates to vary from site to site (if there are test/background branches, separate distributions of SRV are fitted)",
...
if (busted.has_background) {
    ...
    if (busted.do_srv) {
      if (!busted.do_bs_srv && !busted.do_srv_hmm) {
        busted.srv_rate_regex  = "GDD rate category [0-9]+";
        busted.srv_weight_regex = "Mixture auxiliary weight for GDD category [0-9]+";
        busted.background_srv_distribution = regexp.PartitionByRegularExpressions(utility.Keys ((busted.background.bsrel_model[terms.parameters])[terms.global]), {"0" : busted.srv_rate_regex, "1" : busted.srv_weight_regex});


        busted.background_srv_distribution = {
            'rates' : utility.UniqueValues (utility.Map (busted.background_srv_distribution [busted.srv_rate_regex ]  , "_value_", '((busted.background.bsrel_model[terms.parameters])[terms.global])[_value_]')),
            'weights' : utility.UniqueValues (utility.Map (busted.background_srv_distribution [busted.srv_weight_regex ]  , "_value_", '((busted.background.bsrel_model[terms.parameters])[terms.global])[_value_]'))
        };
      }
    }


    PARAMETER_GROUPING + busted.background_srv_distribution["rates"];
    PARAMETER_GROUPING + busted.background_srv_distribution["weights"];

    busted.init_grid_setup (busted.background_srv_distribution);
...
if (busted.has_background) {
    ...
    if (busted.do_srv) {
        if ((!busted.do_bs_srv) && (!busted.do_srv_hmm)) {
            busted.srv_info = Transpose((rate_variation.extract_category_information(busted.background.bsrel_model))["VALUEINDEXORDER"][0])%0;
        }
        io.ReportProgressMessageMD("BUSTED", "main", "* The following rate distribution for site-to-site **synonymous** rate variation was inferred along **background** branches");

        selection.io.report_distribution (busted.srv_info);
        busted.distribution_for_json [busted.SRV.background ] = (utility.Map (utility.Range (busted.synonymous_rate_classes, 0, 1),
                                                                 "_index_",
