...
if (utility.Array1D (tree_annotation[terms.trees.model_list]) < 2) {
    tree_annotation = trees.extract_paml_annotation (tree_annotation[terms.trees.newick_annotated]);
}
...
/* 7. define the GY94 rate matrix; for now each branch will have it's own
   ^paml.ds_name and dN, we will constrain them later */
...
/* 15. Constrain ^paml.ds_name and dN in the tree to based upon different models */
