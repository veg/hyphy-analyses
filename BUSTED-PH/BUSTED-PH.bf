...
",
                               ...
                               terms.io.requirements : "in-frame codon alignment and a phylogenetic tree with exactly one non-trivial branch subset ({} format)"
                              };
...
busted.constrained_bg = "constrained background";
busted.optimized_null_bg = "optimized null background";
...
busted.save_full_lf = estimators.TakeLFStateSnapshot (busted.full_model[terms.likelihood_function]);

if (!busted.run_test) {
    io.ReportProgressMessageMD ("BUSTED", "Results", "No evidence for episodic diversifying positive selection on test branches under the unconstrained model, skipping constrained model fitting");
    busted.json [terms.json.test_results] = busted.ComputeLRT (0, 0);
} else {
    selection.io.startTimer (busted.json [terms.json.timers], "Constrained BUSTED-PH model fitting", 3);
    ...
    selection.io.stopTimer (busted.json [terms.json.timers], "Constrained BUSTED-PH model fitting");
...
estimators.RestoreLFStateFromSnapshot (busted.full_model[terms.likelihood_function], busted.save_full_lf);

// RUN THE BUSTED TEST ON BACKGROUND BRANCHES

busted.inferred_background_distribution = parameters.GetStickBreakingDistribution (busted.background_distribution) % 0; 
busted.run_test = busted.inferred_background_distribution [busted.rate_classes-1][0] > 1 && busted.inferred_background_distribution [busted.rate_classes-1][1] > 0;

if (!busted.run_test) {
    io.ReportProgressMessageMD ("BUSTED", "Results", "No evidence for episodic diversifying positive selection on background branches under the unconstrained model, skipping constrained model fitting");
    busted.json [busted.BG_TEST] = busted.ComputeLRT (0, 0);
} else {
    selection.io.startTimer (busted.json [terms.json.timers], "Constrained BUSTED-PH model fitting background", 4);

    io.ReportProgressMessageMD ("BUSTED", "test-bg", "Performing the constrained background (dN/dS > 1 not allowed on background branches) model fit");
    ...
    (busted.json [busted.json.site_logl])[busted.constrained_bg] = busted.ComputeSiteLikelihoods (busted.full_model[terms.likelihood_function]);
    busted.null_results = estimators.FitExistingLF (busted.full_model[terms.likelihood_function], busted.model_object_map);
    (busted.json [busted.json.site_logl])[busted.optimized_null_bg] = busted.ComputeSiteLikelihoods (busted.full_model[terms.likelihood_function]);
    io.ReportProgressMessageMD ("BUSTED", "test-bg", "* " + selection.io.report_fit (busted.null_results, 9, busted.codon_data_info[terms.data.sample_size]));
    busted.LRT = busted.ComputeLRT (busted.full_model[terms.fit.log_likelihood], busted.null_results[terms.fit.log_likelihood]);
    busted.json [busted.BG_TEST] = busted.LRT;


    selection.io.stopTimer (busted.json [terms.json.timers], "Constrained BUSTED-PH model fitting background");
    ...
    io.ReportProgressMessageMD("BUSTED", "test-bg", "* For *test* branches under the null (no dN/dS > 1 on background branches model), the following rate distribution for branch-site combinations was inferred");
    busted.inferred_test_distribution = parameters.GetStickBreakingDistribution (busted.distribution) % 0;
    selection.io.report_dnds (parameters.GetStickBreakingDistribution (busted.distribution) % 0);
    ...
    if (busted.has_background) {
        busted.inferred_background_distribution = parameters.GetStickBreakingDistribution (busted.background_distribution) % 0;
        io.ReportProgressMessageMD("BUSTED", "test-bg", "* For *background* branches under the null (no dN/dS > 1 on background branches model), the following rate distribution for branch-site combinations was inferred");
        selection.io.report_dnds (busted.inferred_background_distribution);
        busted.distribution_for_json [busted.BG] = utility.Map (utility.Range (busted.rate_classes, 0, 1),
                                                             "_index_",
                                                             "{terms.json.omega_ratio : busted.inferred_background_distribution [_index_][0],
    ...
    selection.io.json_store_lf (busted.json,
                            "Constrained model (background branches)",
                            busted.null_results[terms.fit.log_likelihood],
                            busted.null_results[terms.parameters] + 9 , // +9 comes from CF3x4
                            busted.codon_data_info[terms.data.sample_size],
                            busted.distribution_for_json,
    ...
    (busted.json [busted.json.evidence_ratios])[busted.constrained_bg] = busted.EvidenceRatios ( (busted.json [busted.json.site_logl])[busted.unconstrained],  (busted.json [busted.json.site_logl])[busted.constrained_bg]);
    (busted.json [busted.json.evidence_ratios ])[busted.optimized_null_bg] = busted.EvidenceRatios ( (busted.json [busted.json.site_logl])[busted.unconstrained],  (busted.json [busted.json.site_logl])[busted.optimized_null_bg]);
}

// END RUN BUSTED TEST ON BACKGROUND BRANCHES

    
// RUN THE BUSTED TEST FOR EQUAL DISTRIBUTIONS 
selection.io.startTimer (busted.json [terms.json.timers], "Compare distributions", 5);
...
busted.null_results = estimators.FitExistingLF (busted.full_model[terms.likelihood_function], busted.model_object_map);
io.ReportProgressMessageMD ("BUSTED", "test-shared", "* " + selection.io.report_fit (busted.null_results, 9, busted.codon_data_info[terms.data.sample_size]));

busted.LRT = math.DoLRT ( busted.null_results[terms.fit.log_likelihood], busted.full_model[terms.fit.log_likelihood], utility.Array1D (busted.shared_regime));
busted.json [busted.shared_distributions] = busted.LRT;

io.ReportProgressMessageMD("BUSTED", "test-shared", "* For the shared rates model (same between test and background), the following rate distribution for branch-site combinations was inferred");
busted.inferred_test_distribution = parameters.GetStickBreakingDistribution (busted.distribution) % 0;
selection.io.report_dnds (parameters.GetStickBreakingDistribution (busted.distribution) % 0);
...
selection.io.stopTimer (busted.json [terms.json.timers], "Compare distributions");

// END THE BUSTED TEST FOR EQUAL DISTRIBUTIONS 



console.log ("----\n## Branch-site unrestricted statistical test of episodic diversification and association with phenotype/trait [BUSTED-PH]");
console.log ( "Likelihood ratio test for episodic diversifying positive selection on test branches , **p = " + Format ((busted.json [terms.json.test_results])[terms.p_value], 8, 4) + "**.");

console.log ( "Likelihood ratio test for episodic diversifying positive selection on background branches , **p = " + Format ((busted.json [busted.BG_TEST])[terms.p_value], 8, 4) + "**.");

console.log ( "Likelihood ratio test for differences in distributions between **test** and **background** , **p = " + Format ((busted.json [busted.shared_distributions])[terms.p_value], 8, 4) + "**.");

selection.io.stopTimer (busted.json [terms.json.timers], "Overall");

console.log ("\n\n## Analysis summary (p = " + busted.p_value + ")");

if ((busted.json [terms.json.test_results])[terms.p_value] <= busted.p_value ) {
    if ((busted.json [busted.BG_TEST])[terms.p_value] > busted.p_value) {
        if ((busted.json [busted.shared_distributions])[terms.p_value] <= busted.p_value) {
            console.log ("Selection is associated with the phenotype / trait");
        } else {
            console.log ("Selection is associated with the phenotype / trait, but there is no significant difference between test and background branches in terms of selective pressure");
        }
    } else {
        console.log ("Selection is acting on the branches with the phenotype / trait, but is **also** acting on background branches.");
        if ((busted.json [busted.shared_distributions])[terms.p_value] <= busted.p_value) {
            console.log ("There is a significant difference between test and background branches in terms of selective pressure");
        } else {
            console.log ("There is no significant difference between test and background branches in terms of selective pressure");
        }
        
    }
} else {
    console.log ("There is **no evidence** of episodic diversifying selection on test branches; selection is not associated with phenotype/trait");
}
...
lfunction busted.select_branches(partition_info) {

    io.CheckAssertion("utility.Array1D (`&partition_info`) == 1", "BUSTED-PH only works on a single partition dataset");
    available_models = {};
