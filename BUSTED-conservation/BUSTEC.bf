...
busted.analysis_description = {
                               terms.io.info : 
"BUSTEC (branch-site unrestricted statistical test of episodic conservation) uses a random effects branch-site model fitted 
jointly to all or a subset of tree branches in order to test for alignment-wide evidence of conservation (negative selection). 
Assuming there is evidence of negative selection (i.e. there is an omega < 1),  BUSTED will also perform a quick evidence-ratio 
...
io.ReportProgressMessageMD ("BUSTED", "main", "Performing the full (dN/dS < 1 and dN/dS > 1 allowed) branch-site model fit");
...
busted.run_test = FALSE;
for (busted.i = 1; busted.i < busted.rate_classes; busted.i += 1) {
    busted.run_test = busted.run_test || busted.inferred_test_distribution [busted.i-1][0] < 1 && busted.inferred_test_distribution [busted.i-1][1] > 0;
}
...
if (!busted.run_test) {
    io.ReportProgressMessageMD ("BUSTED", "Results", "No evidence for episodic negative selection under the unconstrained model, skipping constrained model fitting");
    ...
    for (busted.i = 1; busted.i < busted.rate_classes; busted.i += 1) {
        parameters.SetRange (model.generic.GetGlobalParameter (busted.test.bsrel_model , terms.AddCategory (terms.parameters.omega_ratio,busted.i)), terms.range_gte1);
    }
...
console.log ( "Likelihood ratio test for episodic purifying selection, **p = " + Format ((busted.json [terms.json.test_results])[terms.p_value], 8, 4) + "**.");
...
function busted.ComputeLRT (ha, h0) {
    return {terms.LRT : 2*(ha-h0),
            terms.p_value : 1-CChi2 (2*(ha-h0),2*(busted.rate_classes-1))};
}
